    ...
    <div className="mx-auto max-w-6xl px-4 py-10 grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      <div className="space-y-4">
        <div className="relative aspect-video rounded-xl overflow-hidden shadow-md">
          <img src={images[currentImageIndex]} alt="Course" className="object-cover w-full h-full" />
          <button
          ...
          {images.map((img, idx) => (
            <button
              ...
              className={`w-24 aspect-video rounded-md overflow-hidden ring-offset-2 transition ${
                currentImageIndex === idx ? "ring-2 ring-blue-500" : "hover:ring-2 hover:ring-blue-400"
              }`}
            >
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-semibold">{discount}% OFF</span>
        </div>

        <div className="border-t pt-4 space-y-2">
          <p className="text-gray-800 font-semibold text-sm">What you'll get:</p>
          ...
          <div className="flex gap-4 mt-2">
            {["days", "hours", "minutes", "seconds"].map((unit, idx) => (
              <div key={idx} className="flex flex-col items-center text-sm">
                <span className="text-xl font-bold bg-gray-100 px-3 py-1 rounded">{formatNumber(timeLeft[unit])}</span>
